...
class Network():
    ...
    def reset(self, name, iteration):
        ...
        for i in range(1,self.n_nodes + 1):
            ...
            self.tcp_flag[i] = False
            self.tcp_flag_1[i] = False
            self.tcp_flag_count[i] = 30
        ...
        while True:
            ...
            if (elapse_time > 15 and packet.type == 'UDP') or (elapse_time > 20 and packet.type == 'TCP'):
                ...
                if packet.type == 'TCP':
                    ...
                    self.tcp_flag[packet.node] = True
                else:
                ...
                if dest in self.neighbour[node].values():
                    ...
                    if packet.type == 'TCP':
                        if self.links[node] == 3:
                            pass
                        elif not self.tcp_loss[node] and not self.tcp_loss_1[node] and not self.tcp_flag[node] and not self.tcp_flag_1[node]:
                            #print('normal')
                            ...
                            forward_port = min_port
                            packet.forward_port = forward_port
                            if self.tcp_flag_count[node] == 0:
                                self.tcp_flag[node] = False
                                self.tcp_flag_1[node] = False
                                self.tcp_flag_count[node] = 30
                            else:
                                self.tcp_flag_count[node] -= 1
            ...
            if packet.next == None:  # 新的发生包
                ...
                if dest in self.neighbour[node].values():
                    ...
                    #print('self.tcp_loss',self.tcp_loss)
                    #print('self.tcp_loss_1',self.tcp_loss_1)
                    MinQ_port_eval, forward_port, Q_estimate_list = self.agent[node].estimate(dest, receive_port,epsilon)
                    packet.forward_port = forward_port
                    if packet.type == 'TCP':
                        if not self.tcp_loss[node] and not self.tcp_loss_1[node] and not self.tcp_flag[node] and not self.tcp_flag_1[node]:
                            #print('normal')
                            Q_TCP = defaultdict(float)
                            for i in range(1,len(Q_estimate_list)+1):
                                Q_TCP[i] = Q_estimate_list[i-1]
